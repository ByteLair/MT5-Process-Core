...
def main():
    ...
    report = {"r2": float(r2), "mae": float(mae), "n_train": int(len(X_train)), "n_test": int(len(X_test))}
    print(f"[ML] R2={r2:.4f}  MAE={mae:.6f}")

    # Salva artefatos
    joblib.dump(model, MODEL_PKL)
    FEATURES_JSON.write_text(json.dumps(feature_cols, ensure_ascii=False, indent=2), encoding="utf-8")
