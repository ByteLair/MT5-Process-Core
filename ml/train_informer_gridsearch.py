...
drop_cols = ['ts']
numeric_cols = [c for c in df.columns if np.issubdtype(df[c].dtype, np.number) and c not in drop_cols]
...
X_neg = X[y == 0]
y_neg = y[y == 0]
X_pos_upsampled, y_pos_upsampled = resample(X_pos, y_pos, replace=True, n_samples=len(y_neg), random_state=42)
...
for seq_len, d_model, e_layers, dropout, lr in product(param_grid['seq_len'], param_grid['d_model'], param_grid['e_layers'], param_grid['dropout'], param_grid['lr']):
    ...
    for epoch in range(6):  # epochs reduzidos para grid
        ...
        print(f"Epoch {epoch+1}/6 | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Prec: {val_precision:.3f} | Rec: {val_recall:.3f}")
